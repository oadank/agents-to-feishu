...
- v2 复核（2026-09-19 04:1x-04:2x，老大令查引擎日志）：dsh 视觉 ❌→✅（引擎 FINAL tools=13+3959B）；codex 桌面 ❌→✅（rollout-00-46 function_call active_window×2+list_apps×2）、codex 视觉 ❌→✅（rollout-02-01 ocr_image×3+look_image×2）；openakita 桌面 ⏸→✅（openakita.log 02:55:57 active_window+list_apps 实调）。mimo 视觉桥 rt.log+~/.mimocode 引擎侧均无工具痕迹 ❌ 维持交原文。桌面 11✅ 全绿；视觉 10✅1❌(mimo)。教训：判定器/桥日志无痕 ≠ 真没调，引擎侧 session 日志（codex rollout/openakita.log/dsh FINAL）才是完整证据链。
