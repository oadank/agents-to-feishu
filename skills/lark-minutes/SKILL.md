...
## ⛔ 配额红线（老大 2026-09-21 手令，全体 bot 强制，优先级高于本技能一切指引）

飞书妙记转写**不是免费的**：免费版**每人每月 300 分钟**语音转文字（本地上传/会议录制/妙记录音/云文件导入四种场景共用），**AI 智能纪要试用仅 2 篇/人**，每月 1 号只恢复转写分钟数。这烧的是老大本人的账号额度。

1. **未经老大在当次对话里明确点名"走妙记/传妙记"，禁止任何 bot 上传音频/视频到妙记做转写或生成纪要**——不管什么理由（"技能说优先妙记"作废，以本节为准）。
2. 新音频要转文字：**一律先走本地 ASR**（agents-to-feishu 桥 `src/voice/asr.ts`，小米 mimo/whisper 路线，不花飞书额度；bot 侧等价工具：桥 transcribe / 本机 whisper）。本地转不了再**报告老大**征询，不自作主张上传。
3. **读取、检索、编辑已存在的妙记**（给 minute_token 的活）不耗转写额度，正常干。
4. 发现本月额度异常消耗（老大喊"额度没了"），第一时间查最近谁经手过上传（lark-cli minutes 上传记录/妙记列表 `created` 时间对账），如实上报，不许互相甩锅。

违反本红线 = 烧老大真金白银的额度 = 按最严重事故处理。
