/**
 * Reasonix Provider — ACP 协议接入 `reasonix-cli acp` 进程。
 *
 ...
 * - 模型：走 reasonix CLI 自带的网关/模型配置（provider/model 由 CLI 决定）
 *
...
function resolveReasonixCommand(): { command: string; args: string[]; cwd: string } {
  ...
  let command = process.env.CTI_REASONIX_EXEC || '';
  if (!command) {
    for (const exe of candidates) { if (fs.existsSync(exe)) { command = exe; break; } }
  }
  if (!command) command = 'reasonix';
...
function buildSpawnEnv(): NodeJS.ProcessEnv {
  ...
  return {
    ...
    // 注入同款思路：指向真实用户目录（探针模拟 LocalSystem 复现 + 用户环境复验）。
    APPDATA: 'C:\\Users\\oadan\\AppData\\Roaming',
    USERPROFILE: 'C:\\Users\\oadan',
    HOME: 'C:\\Users\\oadan',
  };
}
...
export class ReasonixProvider implements RuntimeProvider {
  ...
  async prepare(): Promise<void> {
    const { command } = resolveReasonixCommand();
    ...
    if (process.platform === 'win32') {
      try {
        ...
        console.warn(`[reasonix] prepare pre-spawn ACP failed (Windows):`, e);
      }
      return;
    }
    try {
      ...
      console.warn(`[reasonix] prepare pre-spawn ACP failed:`, e);
    }
  }

  async resetSession(sessionKey?: string): Promise<void> {
    if (sessionKey) { this.sessions.delete(sessionKey); rtLog(`[reasonix] resetSession key=${sessionKey.slice(0, 8)}`); }
  }
  ...
  async interrupt(): Promise<void> {
    ...
    try {
      ...
      rtLog(`[reasonix] interrupt session=${this.activePrompt.sessionId.slice(0, 8)}`);
    } catch {}
    this.interruptedSessionIds.add(this.activePrompt.sessionId);
    if (this.currentStreamEnd) {
      await this.currentStreamEnd;
      rtLog(`[reasonix] interrupt: current turn fully ended`);
    }
  }
  ...
  private startCleanupTimer(): void {
    ...
    this.cleanupTimer = setInterval(() => {
      ...
      for (const [key, s] of this.sessions) {
        if (ReasonixProvider.IDLE_TIMEOUT_MS > 0 && now - s.lastUsed > ReasonixProvider.IDLE_TIMEOUT_MS) {
          this.sessions.delete(key);
          rtLog(`[reasonix] idle cleanup session ${s.sessionId.slice(0, 8)}`);
        }
      }
    }, 60_000);
  }
  ...
  private handleProcessLost(reason: string): void {
    rtLog(`[reasonix] ⚠️ ${reason} —— 复位常驻把手 + 唤醒在途轮次，下一条消息重建引擎进程`);
    this.child = null;
    ...
    if (stranded) { try { stranded.onDone(`Reasonix 引擎进程已退出，本条消息请重发（已自动复位，下条消息走新进程）`); } catch { /* 单条唤醒失败不阻塞收口 */ } }
    // 票 hand-3 剩余（老大 09-20"别靠监控垃圾维持稳定，把代码写对"）：pending 过去只存
    // resolve，初始化/建会话阶段的等待方进程死了也不醒，只能干等各自兜底超时（prompt 更无超时=永生）。
    this.wakePending(`Reasonix 引擎进程已退出`);
  }
  ...
  private writeStdin(child: ChildProcess, payload: unknown, what: string): boolean {
    ...
    if (!stdin || stdin.destroyed) {
      rtLog(`[reasonix] ⚠️ stdin 不可写(${what}) —— 引擎进程已不在，丢弃本次发送`);
      if (this.child === child) this.handleProcessLost(`Reasonix 引擎 stdin 已关闭（${what}）`);
      return false;
    }
    try {
      stdin.write(JSON.stringify(payload) + '\n', (err) => {
        if (!err) return;
        rtLog(`[reasonix] ⚠️ stdin 异步写失败(${what}): ${err.message}`);
        if (this.child === child) this.handleProcessLost(`Reasonix 引擎 stdin 写入失败（${what}: ${err.message}）`);
      });
      return true;
    } catch (e) {
      rtLog(`[reasonix] ⚠️ stdin 同步写异常(${what}): ${e instanceof Error ? e.message : String(e)}`);
      if (this.child === child) this.handleProcessLost(`Reasonix 引擎 stdin 写入异常（${what}）`);
      return false;
    }
  }
  ...
  private ensureProcess(): Promise<ChildProcess> {
    ...
    this.spawnPromise = new Promise<ChildProcess>((resolve, reject) => {
      rejectSpawn = reject;
      const { command, args, cwd } = resolveReasonixCommand();
      ...
      rtLog(`[reasonix] ACP spawned pid=${child.pid}`);
      child.stderr?.on('data', (c: Buffer) => rtLog(`[reasonix] ACP stderr: ${c.toString().trim().slice(0, 300)}`));
      // 票 hand-3③：stdin 上的 error 没有监听器 = Node 抛 uncaughtException，整个桥一起没。
      child.stdin?.on('error', (err) => {
        rtLog(`[reasonix] ⚠️ ACP stdin error: ${err.message}`);
        if (this.child === child) this.handleProcessLost(`Reasonix 引擎 stdin 异常（${err.message}）`);
      });
      child.on('error', (err) => {
        rtLog(`[reasonix] SPAWN ERROR: ${err.message}`);
      ...
      child.on('exit', (code, signal) => {
        if (this.child === child) {
          this.handleProcessLost(`Reasonix 引擎进程已退出（exit code=${code} signal=${signal}）`);
        } else if (this.spawnPromise) {
          this.spawnPromise = null;
          rejectSpawn(new Error(`Reasonix ACP exited before initialize (code=${code})`));
        }
      });
      child.on('close', (code) => {
        rtLog(`[reasonix] ACP exited code=${code}`);
        if (this.child === child) {
          ...
          // 2026-09-01 修复：同 openclaw——初始化完成前退出时清悬挂 spawnPromise，
          // 否则后续 ensureProcess 永远复用死 promise（initialize timeout 且无法自愈）
          this.spawnPromise = null;
          rejectSpawn(new Error(`Reasonix ACP exited during init (code=${code})`));
        }
      });
      ...
      // reasonix ACP 冷启动给足 60s（不涉及 tsx 编译，比 DSH 快）
      ...
      this.waitResponse(initId, 60_000).then(
        (msg) => {
          ...
          rtLog(`[reasonix] ACP initialized`);
          this.child = child;
          resolve(child);
        },
  ...
  private pruneOldSessions(): void {
    try {
      const root = path.join(os.homedir(), '.reasonix');
      // 仅做提示级清理；openclaw 由 CLI 自身管理会话目录
      if (!fs.existsSync(root)) return;
      rtLog(`[reasonix] session root exists: ${root}`);
    } catch {}
  }
  ...
  async *streamChat(params: StreamChatParams): AsyncGenerator<StreamEvent> {
    ...
    if (this.sessions.size >= ReasonixProvider.MAX_SESSIONS && !this.sessions.has(sessionKey)) {
    ...
    const watchdog = setInterval(() => {
      ...
      if (Date.now() - lastOutput > ReasonixProvider.PROMPT_TIMEOUT_MS) {
        clearInterval(watchdog);
        if (gotText) {
          // 2026-08-30 同步 gemini/hermes 修复：正文已完整流出=结束信号丢失，静默完成不报错
          console.log(`[Reasonix] 超时但正文已完整流出——按正常完成处理`);
          promptHandler.onDone();
        } else {
        promptHandler.onDone(`Reasonix ACP 卡死：连续 ${ReasonixProvider.PROMPT_TIMEOUT_MS / 1000}s 无输出，已中断`);
        }
        rtLog(`[reasonix] watchdog timeout promptId=${promptId}`);
      }
    }, 30_000);
    ...
    rtLog(`[reasonix] prompt sent id=${promptId} session=${session.sessionId.slice(0, 8)}`);
