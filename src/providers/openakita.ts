...
export class OpenAkitaProvider implements RuntimeProvider {
  ...
  private handleProcessLost(reason: string): void {
    rtLog(`[openakita] ⚠️ ${reason} —— 复位常驻把手 + 唤醒在途轮次，下一条消息重建引擎进程`);
    this.child = null;
  ...
  private writeStdin(child: ChildProcess, payload: unknown, what: string): boolean {
    ...
    if (!stdin || stdin.destroyed) {
      rtLog(`[openakita] ⚠️ stdin 不可写(${what}) —— 引擎进程已不在，丢弃本次发送`);
      if (this.child === child) this.handleProcessLost(`OpenAkita 引擎 stdin 已关闭（${what}）`);
      return false;
    }
    try {
      stdin.write(JSON.stringify(payload) + '\n', (err) => {
        if (!err) return;
        rtLog(`[openakita] ⚠️ stdin 异步写失败(${what}): ${err.message}`);
        if (this.child === child) this.handleProcessLost(`OpenAkita 引擎 stdin 写入失败（${what}: ${err.message}）`);
      });
      return true;
    } catch (e) {
      rtLog(`[openakita] ⚠️ stdin 同步写异常(${what}): ${e instanceof Error ? e.message : String(e)}`);
      if (this.child === child) this.handleProcessLost(`OpenAkita 引擎 stdin 写入异常（${what}）`);
      return false;
    }
  }
  ...
  private ensureProcess(): Promise<ChildProcess> {
    ...
    this.spawnPromise = new Promise<ChildProcess>((resolve, reject) => {
      ...
      child.stdin?.on('error', (err) => {
        rtLog(`[openakita] ⚠️ ACP stdin error: ${err.message}`);
        if (this.child === child) this.handleProcessLost(`OpenAkita 引擎 stdin 异常（${err.message}）`);
      });
      child.on('error', (err) => {
      ...
      child.on('exit', (code, signal) => {
        if (this.child === child) {
          this.handleProcessLost(`OpenAkita 引擎进程已退出（exit code=${code} signal=${signal}）`);
        } else if (this.spawnPromise) {
          this.spawnPromise = null;
          rejectSpawn(new Error(`OpenAkita ACP exited before initialize (code=${code})`));
        }
      });
      child.on('close', (code) => {
  ...
  async *streamChat(params: StreamChatParams): AsyncGenerator<StreamEvent> {
    ...
    if (isNewSession && !params.freshSession && params.history && params.history.length > 0) {
      rtLog(`[openakita] engine session lost (shadow ${params.history.length}) → auto /new`);
    ...
    if (historyText) {
      ...
      rtLog(`[openakita] new session: injected ${params.history?.length ?? 0} history turns`);
    }
    ...
    this.waitResponse(promptId).then(
      ...
      (err: unknown) => {
        ...
        promptHandler.onDone(/timeout/i.test(em) ? "OpenAkita 引擎本轮未回 prompt 响应（请求超时）" : (em || "OpenAkita ACP prompt 响应异常"));
      },
    );
