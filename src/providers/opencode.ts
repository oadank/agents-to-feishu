/**
 * Opencode Provider — ACP 协议接入 `opencode acp` 进程。
 *
 ...
 * - 模型：走 opencode CLI 自带的网关/模型配置（provider/model 由 CLI 决定）
 *
...
function resolveOpencodeCommand(): { command: string; args: string[]; cwd: string } {
  const candidates = [
    'C:\\Users\\oadan\\AppData\\Roaming\\npm\\node_modules\\opencode-ai\\node_modules\\opencode-windows-x64\\bin\\opencode.exe',
    'C:\\Users\\oadan\\AppData\\Roaming\\npm\\node_modules\\opencode-ai\\node_modules\\opencode-windows-x64-baseline\\bin\\opencode.exe',
  ];
  let command = process.env.CTI_OPENCODE_EXEC || '';
  if (!command) {
...
export class OpencodeProvider implements RuntimeProvider {
  ...
  async prepare(): Promise<void> {
    ...
    if (process.platform === 'win32') {
      try {
        ...
        console.warn(`[opencode] prepare pre-spawn ACP failed (Windows):`, e);
      }
      return;
    }
    try {
      ...
      console.warn(`[opencode] prepare pre-spawn ACP failed:`, e);
    }
  }
  ...
  async interrupt(): Promise<void> {
    ...
    try {
      ...
      rtLog(`[opencode] interrupt session=${this.activePrompt.sessionId.slice(0, 8)}`);
    } catch {}
    this.interruptedSessionIds.add(this.activePrompt.sessionId);
    if (this.currentStreamEnd) {
      await this.currentStreamEnd;
      rtLog(`[opencode] interrupt: current turn fully ended`);
    }
  }
  ...
  private startCleanupTimer(): void {
    ...
    this.cleanupTimer = setInterval(() => {
      ...
      for (const [key, s] of this.sessions) {
        if (OpencodeProvider.IDLE_TIMEOUT_MS > 0 && now - s.lastUsed > OpencodeProvider.IDLE_TIMEOUT_MS) {
          this.sessions.delete(key);
          rtLog(`[opencode] idle cleanup session ${s.sessionId.slice(0, 8)}`);
        }
      }
    }, 60_000);
  }
  ...
  private handleProcessLost(reason: string): void {
    rtLog(`[opencode] ⚠️ ${reason} —— 复位常驻把手 + 唤醒在途轮次，下一条消息重建引擎进程`);
    this.child = null;
    ...
    if (stranded) { try { stranded.onDone(`OpenCode 引擎进程已退出，本条消息请重发（已自动复位，下条消息走新进程）`); } catch { /* 单条唤醒失败不阻塞收口 */ } }
    // 票 hand-3 剩余（老大 09-20"别靠监控垃圾维持稳定，把代码写对"）：pending 过去只存
    // resolve，初始化/建会话阶段的等待方进程死了也不醒，只能干等各自兜底超时（prompt 更无超时=永生）。
    this.wakePending(`OpenCode 引擎进程已退出`);
  }
  ...
  private writeStdin(child: ChildProcess, payload: unknown, what: string): boolean {
    ...
    if (!stdin || stdin.destroyed) {
      rtLog(`[opencode] ⚠️ stdin 不可写(${what}) —— 引擎进程已不在，丢弃本次发送`);
      if (this.child === child) this.handleProcessLost(`OpenCode 引擎 stdin 已关闭（${what}）`);
      return false;
    }
    try {
      stdin.write(JSON.stringify(payload) + '\n', (err) => {
        if (!err) return;
        rtLog(`[opencode] ⚠️ stdin 异步写失败(${what}): ${err.message}`);
        if (this.child === child) this.handleProcessLost(`OpenCode 引擎 stdin 写入失败（${what}: ${err.message}）`);
      });
      return true;
    } catch (e) {
      rtLog(`[opencode] ⚠️ stdin 同步写异常(${what}): ${e instanceof Error ? e.message : String(e)}`);
      if (this.child === child) this.handleProcessLost(`OpenCode 引擎 stdin 写入异常（${what}）`);
      return false;
    }
  }
  ...
  private ensureProcess(): Promise<ChildProcess> {
    ...
    this.spawnPromise = new Promise<ChildProcess>((resolve, reject) => {
      rejectSpawn = reject;
      const { command, args, cwd } = resolveOpencodeCommand();
      ...
      rtLog(`[opencode] ACP spawned pid=${child.pid}`);
      child.stderr?.on('data', (c: Buffer) => rtLog(`[opencode] ACP stderr: ${c.toString().trim().slice(0, 300)}`));
      // 票 hand-3③：stdin 上的 error 没有监听器 = Node 抛 uncaughtException，整个桥一起没。
      child.stdin?.on('error', (err) => {
        rtLog(`[opencode] ⚠️ ACP stdin error: ${err.message}`);
        if (this.child === child) this.handleProcessLost(`OpenCode 引擎 stdin 异常（${err.message}）`);
      });
      child.on('error', (err) => {
        rtLog(`[opencode] SPAWN ERROR: ${err.message}`);
      ...
      child.on('exit', (code, signal) => {
        if (this.child === child) {
          this.handleProcessLost(`OpenCode 引擎进程已退出（exit code=${code} signal=${signal}）`);
        } else if (this.spawnPromise) {
          this.spawnPromise = null;
          rejectSpawn(new Error(`OpenCode ACP exited before initialize (code=${code})`));
        }
      });
      child.on('close', (code) => {
        rtLog(`[opencode] ACP exited code=${code}`);
        if (this.child === child) {
          ...
          this.spawnPromise = null;
          rejectSpawn(new Error(`OpenCode ACP exited during init (code=${code})`));
        }
      });
      ...
      // opencode ACP 冷启动给足 60s（不涉及 tsx 编译，比 DSH 快）
      const timeout = setTimeout(() => {
        if (!this.child) { try { child.kill('SIGTERM'); } catch {} reject(new Error('Opencode ACP initialize timeout')); }
      }, 60_000);

      this.waitResponse(initId, 60_000).then(
        (msg) => {
          clearTimeout(timeout);
          if (msg.error) { reject(new Error('Opencode ACP initialize failed')); return; }
          rtLog(`[opencode] ACP initialized`);
          this.child = child;
          resolve(child);
        },
  ...
  private async createSession(cwd: string): Promise<AcpSession> {
    ...
    if (!msg.result) throw new Error('Opencode ACP session/new failed');
    const sessionId = (msg.result as Record<string, unknown>).sessionId as string | undefined;
    if (!sessionId) throw new Error('Opencode ACP session/new: missing sessionId');

    rtLog(`[opencode] session/new OK: ${sessionId.slice(0, 8)}`);
  ...
  private pruneOldSessions(): void {
    try {
      ...
      rtLog(`[opencode] session root exists: ${root}`);
    } catch {}
  }
  ...
  async *streamChat(params: StreamChatParams): AsyncGenerator<StreamEvent> {
    ...
    if (this.sessions.size >= OpencodeProvider.MAX_SESSIONS && !this.sessions.has(sessionKey)) {
    ...
    if (!session || params.freshSession || sessionInterrupted) {
      ...
      try {
        ...
        yield { type: 'error', message: `Opencode ACP 会话创建失败: ${e instanceof Error ? e.message : String(e)}` };
        yield { type: 'done' };
        return;
      }
    }
    ...
    if (isNewSession && !params.freshSession && params.history && params.history.length > 0) {
      rtLog(`[opencode] engine session lost (shadow ${params.history.length}) → auto /new`);
    ...
    if (historyText) {
      ...
      rtLog(`[opencode] new session: injected ${params.history?.length ?? 0} history turns`);
    }
    ...
    this.waitResponse(promptId).then(
      ...
      (err: unknown) => {
        ...
        promptHandler.onDone(/timeout/i.test(em) ? "OpenCode 引擎本轮未回 prompt 响应（请求超时）" : (em || "OpenCode ACP prompt 响应异常"));
      },
    );

    const watchdog = setInterval(() => {
      if (settled) { clearInterval(watchdog); return; }
      if (Date.now() - lastOutput > OpencodeProvider.PROMPT_TIMEOUT_MS) {
        clearInterval(watchdog);
        if (gotText) {
          // 2026-08-30 同步 gemini/hermes 修复：正文已完整流出=结束信号丢失，静默完成不报错
          console.log(`[Opencode] 超时但正文已完整流出——按正常完成处理`);
          promptHandler.onDone();
        } else {
        promptHandler.onDone(`Opencode ACP 卡死：连续 ${OpencodeProvider.PROMPT_TIMEOUT_MS / 1000}s 无输出，已中断`);
        }
        rtLog(`[opencode] watchdog timeout promptId=${promptId}`);
      }
    }, 30_000);
    ...
    rtLog(`[opencode] prompt sent id=${promptId} session=${session.sessionId.slice(0, 8)}`);
