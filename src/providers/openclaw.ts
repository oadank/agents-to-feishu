...
export class OpenClawProvider implements RuntimeProvider {
  ...
  private startCleanupTimer(): void {
    ...
    this.cleanupTimer = setInterval(() => {
      ...
      for (const [key, s] of this.sessions) {
        if (OpenClawProvider.IDLE_TIMEOUT_MS > 0 && now - s.lastUsed > OpenClawProvider.IDLE_TIMEOUT_MS) {
  ...
  private handleProcessLost(reason: string): void {
    rtLog(`[openclaw] ⚠️ ${reason} —— 复位常驻把手 + 唤醒在途轮次，下一条消息重建引擎进程`);
    this.child = null;
  ...
  private writeStdin(child: ChildProcess, payload: unknown, what: string): boolean {
    ...
    if (!stdin || stdin.destroyed) {
      rtLog(`[openclaw] ⚠️ stdin 不可写(${what}) —— 引擎进程已不在，丢弃本次发送`);
      if (this.child === child) this.handleProcessLost(`OpenClaw 引擎 stdin 已关闭（${what}）`);
      return false;
    }
    try {
      stdin.write(JSON.stringify(payload) + '\n', (err) => {
        if (!err) return;
        rtLog(`[openclaw] ⚠️ stdin 异步写失败(${what}): ${err.message}`);
        if (this.child === child) this.handleProcessLost(`OpenClaw 引擎 stdin 写入失败（${what}: ${err.message}）`);
      });
      return true;
    } catch (e) {
      rtLog(`[openclaw] ⚠️ stdin 同步写异常(${what}): ${e instanceof Error ? e.message : String(e)}`);
      if (this.child === child) this.handleProcessLost(`OpenClaw 引擎 stdin 写入异常（${what}）`);
      return false;
    }
  }
  ...
  private ensureProcess(): Promise<ChildProcess> {
    ...
    this.spawnPromise = new Promise<ChildProcess>((resolve, reject) => {
      ...
      child.stdin?.on('error', (err) => {
        rtLog(`[openclaw] ⚠️ ACP stdin error: ${err.message}`);
        if (this.child === child) this.handleProcessLost(`OpenClaw 引擎 stdin 异常（${err.message}）`);
      });
      child.on('error', (err) => {
      ...
      child.on('exit', (code, signal) => {
        if (this.child === child) {
          this.handleProcessLost(`OpenClaw 引擎进程已退出（exit code=${code} signal=${signal}）`);
        } else if (this.spawnPromise) {
          this.spawnPromise = null;
          rejectSpawn(new Error(`OpenClaw ACP exited before initialize (code=${code})`));
        }
      });
      child.on('close', (code) => {
  ...
  async *streamChat(params: StreamChatParams): AsyncGenerator<StreamEvent> {
    ...
    if (isNewSession && !params.freshSession && params.history && params.history.length > 0) {
      rtLog(`[openclaw] engine session lost (shadow ${params.history.length}) → auto /new`);
    ...
    if (historyText) {
      ...
      rtLog(`[openclaw] new session: injected ${params.history?.length ?? 0} history turns`);
    }
